    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $prefix = substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 1, 3);
            $randomNo =  substr(str_shuffle('1234567890'), 1, 2);
            $business_id = $business->id;
            $location_id =  $new_location->id;
            $secret_id =  $prefix . '-' . $randomNo . $business_id . $location_id;

            $business->secret_key =  $secret_id;
            $new_location->secret_key = $secret_id;
            $business->save();
            $new_location->save();
