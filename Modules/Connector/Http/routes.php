...
Route::get('connector/api/hello', function () {
	return "Hello";
});
...
Route::group(['prefix' => 'connector/api', 'namespace' => 'Modules\Connector\Http\Controllers\Api'], function () {
	Route::get('business-location-verify', 'BusinessLocationController@businesslocation');
	Route::get('business-verify', 'CommonResourceController@verifyBusiness');
	Route::get('owner-id/{id}', 'UserController@owner');

	// Post offline product Data

	Route::post('recieve-offline-products' , 'ProductController@offlineProductSave');

	Route::get('invoice-schemes', 'InvoiceController@schemesIndex');
	Route::get('invoice-layouts', 'InvoiceController@layoutsIndex');

	Route::get('location-permission', 'PermissionController@locationPermission');
	Route::get('default-permissions', 'PermissionController@defaultPermissions');
	Route::get('list-users', 'UserController@listUsers');
	Route::get('list-roles', 'UserController@listRoles');

	Route::resource('product', 'ProductController', ['only' => ['index', 'show']]);
	Route::get('list-products', 'ProductController@productsForSync');
	Route::get('list-variation-templates', 'SyncTablesController@variationsTemplates');

	Route::get('list-contacts', 'ContactController@index');
	Route::get('list-invoice_schemes', 'InvoiceController@schemesIndex');
	Route::get('list-invoice_layouts', 'InvoiceController@layoutsIndex');
	Route::get('list-categories', 'CategoryController@listCategories');
	Route::get('list-units', 'UnitController@listUnits');
	Route::get('list-brands', 'BrandController@index');
	Route::get('list-warranties', 'SyncTablesController@warranty');
	Route::get('list-expense_categories', 'ExpenseController@listExpenseCategories');
	Route::get('list-notification_templates', 'SyncTablesController@notifications');
	Route::get('list-types_of_services', 'TypesOfServiceController@index');
	Route::get('list-tax_rates', 'TaxController@index');
	Route::get('purchase-transaction', 'SyncTablesController@transaction');

	Route::resource('table', 'TableController', ['only' => ['index', 'show']]);
	Route::get('list-modifiers', 'ProductController@modifiers');
});
