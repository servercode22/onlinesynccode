...
class ProductController extends ApiController
{
        ...
        // Offline Product Save
        
    public function offlineProductSave(Request $request){
        $output = [];
        try {
        DB::beginTransaction();
        // return $request->all();
        foreach($request['data'] as $product){
            $product_without = Arr::except($product , ['product_variations' , 'product_locations', 'sync_status', 'image_url', 'ai_id']);
            // dd($product_without);
            DB::table('products')->updateOrInsert(['id' => $product_without['id']] , $product_without);
            // Product Variations
            foreach($product['product_variations'] as $product_variation){
                $product_variation_without = Arr::except($product_variation ,['variations']);
                // Save Product Variations
                DB::table('product_variations')->updateOrInsert(
                    ['id' => $product_variation['id']] , 
                    $product_variation_without
                );
                //Variations
                foreach($product_variation['variations'] as $variation){
                    $variation_without = Arr::except($variation , ['sync_status' , 'variation_location_details', 'ai_id','media', 'discounts']);
                    // Save Variations
                    // dd($variation_without);
                    DB::table('variations')->updateOrInsert(
                        ['id' => $variation_without['id']],
                        $variation_without
                    );
                    // variation Location Details
                    foreach($variation['variation_location_details'] as $vld){
                        $vld_without = Arr::except($vld , ['sync_status' , 'transactions', 'purchase_lines', 'ai_id']);
                        // Save variation_location_details
                        DB::table('variation_location_details')->updateOrInsert(['id' => $vld['id']] , $vld_without);
                        
                        // Transactions
                        foreach($vld['transactions'] as $transaction){
                            $transaction_without = Arr::except($transaction , ['sync_status' , 'ai_id', 'transaction_payments']);
                            // Save Transactions
                            DB::table('transactions')->updateOrInsert(
                                ['id' => $transaction_without['id']],
                                $transaction_without
                            );
                            // Transaction Payments
                            foreach($transaction['transaction_payments'] as $transaction_payment){
                                if(!is_null($transaction_payment)){
                                    $transaction_payment_without = Arr::except($vld , ['sync_status' , 'ai_id']);
                                    DB::table('transaction_payments')->updateOrInsert(
                                        ['id'=>$transaction_payment_without['id']] ,
                                        $transaction_payment_without
                                    );
                                }
                            }
                        ...
                        foreach($vld['purchase_lines'] as $pl){
                            $pl_without = Arr::except($pl , ['ai_id' , 'sync_status']);
    ...
    public function productsForSync()
    {
    ...
    public function modifiers()
    {
        ...
        $with = ['modifier_products', 'product_variations.variations'];
        $query = Product::where('business_id', $business_id)->where('type', 'modifier');
        $modifers = $query->with($with)->get();
        return CommonResource::collection($modifers);
    }
    ...
    //         return json_encode($msg);
    //     }
    //     // return response()->json([request('business_id'), request('location_id')]);
