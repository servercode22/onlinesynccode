...
class BusinessLocationController extends ApiController
{
    ...
    public function businesslocation(request $request)
    {
        ...
        if (!$request->has('business_id') || !$request->has('location_pin')) {
            $msg = ['error' => 'Unauthorized'];
            return json_encode($msg);
        }
        $location = BusinessLocation::where('secret_key', $location_pin)->where('business_id', $business_id)->first();
        if (!$location) {
            $msg['error'] = "Location not found";
        }
