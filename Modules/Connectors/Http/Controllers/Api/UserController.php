...
class UserController extends ApiController
{
    ...
    public function listUsers()
    {
        ...
        foreach ($users as $key => $value) {
            ...
            if (!(User::can_user_access_this_location($location_id, $business_id, $value->id) == true)) {
                unset($users[$key]);
            } else {
                $value['model_has_permissions'] = DB::table('model_has_permissions')->where('model_type', 'App\User')->where('model_id', $value->id)->get();
                foreach ($value['model_has_permissions'] as $mp_key => $mp_value) {
                    $mp_value->permissions =  DB::table('permissions')->where('id', $mp_value->permission_id)->first();
                }
            }
        }
    ...
    function listRoles()
    {
        ...
        $roles = DB::table('roles')->where('business_id', $business_id)->get();
        foreach ($roles as $role) {
            $role->model_has_roles = DB::table('model_has_roles')->where('role_id', $role->id)->get();
        }

        foreach ($roles as $role) {
            $role->role_has_permissions = DB::table('role_has_permissions')->where('role_id', $role->id)->get();
            foreach ($role->role_has_permissions as $rhp) {
                $rhp->permissions =  DB::table('permissions')->where('id', $rhp->permission_id)->first();
            }
        }
        return $roles;
    }
